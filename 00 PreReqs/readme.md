...
1. Navigate to and login to the Azure portal (azure.portal.com).  If you're using a guest account, you may want to use a private browser session.  Login with the credentials.  If you're using a guest account, you'll get prompted for a new password.  Change the password and choose "Skip for now (14 days required)".  After you are logged in, start Azure Cloud Shell by clicking the  >_ icon in top right.  
